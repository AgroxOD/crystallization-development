...
function calcAverage(): number {
  ...
  const scores: number[] = data.tasks.map((t: any) => t.final_score ?? 0);
...
function updateReadme(percent: number) {
  ...
  text = text.replace(/crystallization-\d+%25/, `crystallization-${percent}%25`);
  text = text.replace(/\*\*Current Level:\*\* \d+%/, `**Current Level:** ${percent}%`);
