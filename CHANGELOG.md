...
- Минимальный REST API сервер `src/apiServer.ts`
- Скрипт `scripts/api_integration_example.ts` для интеграции через API
- Команда `crystal:api` для запуска сервера

### Changed

- ROADMAP отмечает завершение задачи по REST API
- README дополнен разделом о REST API
- AGENTS.md упоминает новый пример интеграции

## [0.3.28] - 2025-11-15

### Added

- Документ `docs/rest_api_usage_ru.md` с расширенной инструкцией по работе с локальным REST API сервером
...
- Скрипт `crystal:auto-update` для обновления устаревших репозиториев через GitHub API

### Changed

- ROADMAP отмечает выполнение задачи автообновления
- README содержит раздел о новой команде
- AGENTS.md дополнен упоминанием автообновления

## [0.3.31] - 2025-11-18

### Added

- Пример curl-запроса для получения списка коммитов в `docs/github_api_usage.md` и README

### Changed

- ROADMAP теперь содержит пункт о curl-примере
- AGENTS.md обновлён соответствующей строкой

## [0.3.32] - 2025-11-19

### Added

- Новый пункт Roadmap о реализации скрипта `crystal:loop`

### Changed

- README, ROADMAP и AGENTS теперь упоминают запланированный скрипт `crystal:loop`

## [0.3.33] - 2025-11-20

### Added

- Реализован скрипт `crystal:loop` для автоматического запуска итераций и
  соответствующая npm-команда.

### Changed

- Обновлены README, ROADMAP и AGENTS в связи с появлением скрипта `crystal:loop`.
